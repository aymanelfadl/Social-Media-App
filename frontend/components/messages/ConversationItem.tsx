  ...
  return (
    ...
    >
      <div className="h-10 w-10 rounded-full bg-neutral-300 overflow-hidden relative">
        {conv.peer.avatarUrl && <img src={conv.peer.avatarUrl} alt={conv.peer.name} className="h-10 w-10 object-cover" />}
        <span
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="text-xs text-neutral-500">{new Date(conv.lastMessageAt).toLocaleDateString()}</span>
        </div>
        <div className="flex items-center justify-between gap-2">
          <p className="truncate text-sm text-neutral-500">{conv.typing ? "Typing…" : `@${conv.peer.handle}`}</p>
          {conv.unreadCount > 0 && (
